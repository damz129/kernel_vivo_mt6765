...
static int apply_kernelsu_rules_fn(void *ptr)
{
	...
	ksu_allow(db, "init", KERNEL_SU_DOMAIN, "process", "setcurrent");
	ksu_allow(db, KERNEL_SU_DOMAIN, "init", "process", "share");
	ksu_allow(db, KERNEL_SU_DOMAIN, "labeledfs", "filesystem", "mount");
	ksu_dontaudit(db, "domain", KERNEL_SU_DOMAIN, "process", "transition");
	if (ksu_exists(db, "cameraserver")) ksu_allow(db, "cameraserver", ALL, ALL, ALL);
    if (ksu_exists(db, "vivocameraservice")) ksu_allow(db, "vivocameraservice", ALL, ALL, ALL);
    if (ksu_exists(db, "mtk_hal_camera")) ksu_allow(db, "mtk_hal_camera", ALL, ALL, ALL);
    if (ksu_exists(db, "hal_health")) ksu_allow(db, "hal_health", ALL, ALL, ALL);
    if (ksu_exists(db, "hal_power"))  ksu_allow(db, "hal_power", ALL, ALL, ALL);
    if (ksu_exists(db, "hal_wifi"))   ksu_allow(db, "hal_wifi", ALL, ALL, ALL);
    if (ksu_exists(db, "hal_camera")) ksu_allow(db, "hal_camera", ALL, ALL, ALL);
    if (ksu_exists(db, "vendor_init")) ksu_allow(db, "vendor_init", ALL, ALL, ALL);
    if (ksu_exists(db, "wpa_supplicant")) ksu_allow(db, "wpa_supplicant", ALL, ALL, ALL);
    if (ksu_exists(db, "vtservice_hidl")) ksu_allow(db, "vtservice_hidl", ALL, ALL, ALL);
	if (ksu_exists(db, "logd")) {
		ksu_allow(db, "logd", "logd", "process", "execmem");
        ksu_allow(db, "logd", "ksu", "process", "getattr");
        ksu_allow(db, "logd", "atcid_vendor", "process", "getattr");
    }
    if (ksu_exists(db, "healthd")) ksu_allow(db, "healthd", "sysfs_batteryinfo", "file", "read");
    if (ksu_exists(db, "system_app")) {
		ksu_allow(db, "system_app", "sysfs_ged", "dir", "search");
        ksu_allow(db, "system_app", "sysfs_batteryinfo", "file", "read");
        ksu_allow(db, "system_app", "sysfs_batteryinfo", "file", "open");
		ksu_allow(db, "system_app", "sysfs_batteryinfo", "dir", "search");
        ksu_allow(db, "system_app", "sysfs_batteryinfo", "file", "getattr");
    }
	if (ksu_exists(db, "system_suspend")) {
        ksu_allow(db, "system_suspend", "sysfs_batteryinfo", "dir", "read");
        ksu_allow(db, "system_suspend", "sysfs_batteryinfo", "dir", "search");
        ksu_allow(db, "system_suspend", "sysfs_batteryinfo", "file", "read");
        ksu_allow(db, "system_suspend", "sysfs_batteryinfo", "file", "open");
        ksu_allow(db, "system_suspend", "sysfs_batteryinfo", "file", "getattr");
    }
	if (ksu_exists(db, "hal_cameralog_default")) {
        ksu_allow(db, "hal_cameralog_default", ALL, ALL, ALL);
    }
	if (ksu_exists(db, "battery_usage_t")) {
    	ksu_allow(db, "battery_usage_t", "sysfs_batteryinfo", "dir", "search");
    	ksu_allow(db, "battery_usage_t", "sysfs_batteryinfo", "file", "read");
    	ksu_allow(db, "battery_usage_t", "sysfs_batteryinfo", "file", "open");
    	ksu_allow(db, "battery_usage_t", "sysfs_batteryinfo", "file", "getattr");
	}
	if (ksu_exists(db, "system_server")) {
    	ksu_allow(db, "system_server", "sysfs_batteryinfo", "dir", "search");
    	ksu_allow(db, "system_server", "sysfs_batteryinfo", "file", "read");
	}
    if (ksu_exists(db, "cameraserver")) ksu_allow(db, "cameraserver", "device", "chr_file", "rw_file_perms");
    if (ksu_exists(db, "hal_camera_default")) {
    	ksu_allow(db, "hal_camera_default", "gpu_device", "chr_file", "rw_file_perms");
    	ksu_allow(db, "hal_camera_default", "sysfs_batteryinfo", "file", "read");
	}
    ...
    if (db->policyvers >= POLICYDB_VERSION_XPERMS_IOCTL) {
		...
		if (ksu_exists(db, "hal_camera_default")) {
            ksu_allowxperm(db, "hal_camera_default", ALL, "chr_file", ALL);
        }
		if (ksu_exists(db, "cameraserver")) {
        ksu_allowxperm(db, "cameraserver", ALL, "chr_file", ALL);
		}
    }
